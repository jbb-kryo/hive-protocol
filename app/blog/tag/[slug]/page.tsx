...
import { useBlog, useBlogTag, useBlogTags, BlogPost } from '@/hooks/use-blog'
...
export default function BlogTagPage() {
  ...
  const {
    ...
    totalCount,
    totalPages,
  } = useBlog({ tag: slug, page: currentPage })
  ...
  const relatedTags = allTags.filter((t) => t.slug !== slug).slice(0, 10)
      ...
      <section className="pt-32 pb-12 px-4">
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <Breadcrumbs
              customItems={[
                ...
                { label: tag?.name || slug, href: `/blog/tag/${slug}` },
              ]}
              className="mb-6"
            />
            ...
            <h1 className="text-3xl sm:text-4xl font-bold mb-4">
              Posts tagged &quot;{tag?.name || slug}&quot;
            </h1>

            {tag?.description && (
              <p className="text-lg text-muted-foreground max-w-3xl mb-8">{tag.description}</p>
            )}
          </motion.div>
          ...
          {relatedTags.length > 0 && (
            ...
            >
              <h2 className="text-xl font-semibold mb-4">Explore More Tags</h2>
              <div className="flex flex-wrap gap-2">
                {relatedTags.map((t) => (
                  <Link key={t.slug} href={`/blog/tag/${t.slug}`}>
                    <Badge
                      variant="outline"
                      className="gap-1 hover:bg-secondary transition-colors cursor-pointer"
                    >
